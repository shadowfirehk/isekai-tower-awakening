...
export function EarthRegionScreen({save,onBack,onEnter,onCommitted}:Props){
  ...
  return <section className="earth-region phase-screen" aria-label="地球區域完整進度">
    ...
    <div className="earth-layout">
      <nav className="tier-road" aria-label="十二階地球路線">{EARTH_TIER_IDS.map(id=>{const item=EARTH_TIER_CONFIGS[id],state=save.earthRegion.tierProgress[id];return <button key={id} className={`${selected===id?'active':''} ${state.cleared?'cleared':state.unlocked?'unlocked':'locked'}`} onClick={()=>state.unlocked&&setSelected(id)} disabled={!state.unlocked}><span>{String(item.order+1).padStart(2,'0')}</span><i>{state.cleared?<Check/>:state.unlocked?<Shield/>:<LockKeyhole/>}</i><b>{item.name}</b><small>{id}</small></button>;})}</nav>
      <main className="tier-detail">
        <div className="tier-hero"><span>{tier.englishName} · TIER {String(tier.order+1).padStart(2,'0')}</span><h2>{tier.dungeonName}</h2><p>25 WAVES · 頭目波 05 / 10 / 15 / 20 / 25</p><em>{progress.cleared?'CLEARED':progress.unlocked?'AVAILABLE':'LOCKED'}</em></div>
        ...
        <div className="boss-card"><Crown/><span><small>FINAL BOSS · TRAIT</small><h3>{tier.bossName}</h3><p>{tier.bossTrait} · HP ×{tier.hpMultiplier} · DEF ×{tier.defenseMultiplier} · SPD ×{tier.speedMultiplier}</p></span></div>
        ...
        <section className="loadout-builder"><header><div><TowerControl/><span><small>EARTH LOADOUT</small><b>戰前編成 · 最多 3 種炮塔</b></span></div><em>{save.earthLoadout.length} / 3</em></header><div>{owned.map(entry=>{const data=getTowerById(entry.towerID),active=save.earthLoadout.includes(entry.towerID);return <button key={entry.towerID} className={active?'active':''} onClick={()=>toggle(entry.towerID)}><i>{active?<Check/>:<TowerControl/>}</i><span><b>{data?.name}</b><small>{data?.rarity} · Lv.{entry.level} · {entry.stars}★</small></span></button>;})}</div></section>

...
export interface WaveData { wave:number; boss:boolean; bossMultiplier:number; groups:WaveGroup[]; }
const BOSS_MULTIPLIERS=[1,1.5,2.2,3,4];

export function createWaves(tierOrder=-1):WaveData[] {
  return Array.from({length:25},(_,index)=>{
    const wave=index+1,boss=wave%5===0,act=Math.ceil(wave/5),groups:WaveGroup[]=[];
    if (boss) {
      groups.push({enemyId:tierOrder>=4?'EARTH_ELITE':'EARTH_GRUNT',count:2+act,interval:.45});
      if (tierOrder>=5) groups.push({enemyId:tierOrder%2?'EARTH_REGENERATOR':'EARTH_SHIELDED',count:Math.max(1,act-2),interval:.55});
      groups.push({enemyId:'EARTH_BOSS',count:1,interval:0});
    } else {
      groups.push({enemyId:'EARTH_GRUNT',count:3+act+(wave%3),interval:.5});
      if (wave>=6) groups.push({enemyId:'EARTH_RUNNER',count:1+Math.floor(act/2),interval:.36});
      if (wave>=11) groups.push({enemyId:tierOrder>=2?'EARTH_ELITE':'EARTH_TANK',count:Math.max(1,act-1),interval:.7});
      if (wave>=16&&tierOrder>=3) groups.push({enemyId:tierOrder%2?'EARTH_REGENERATOR':'EARTH_SHIELDED',count:1+Math.floor(tierOrder/5),interval:.72});
      if (wave>=21&&tierOrder>=6) groups.push({enemyId:'EARTH_SUPPORT',count:1,interval:.65});
    }
    return {wave,boss,bossMultiplier:boss?BOSS_MULTIPLIERS[act-1]:1,groups};
  });
}

export interface DungeonConfig {
  id:string; name:string; englishName:string; realm:RealmType; tierID:EarthTierId|null; tierOrder:number;
  totalWaves:number; startingBaseHP:number; deploymentCap:number; hpMultiplier:number; defenseMultiplier:number; speedMultiplier:number;
  bossName:string; bossTrait:string; waves:WaveData[];
}
export const TUTORIAL_WAVES=createWaves(-1);
export const EARTH_TUTORIAL_DUNGEON:DungeonConfig={id:'EARTH_TUTORIAL_001',name:'新手試煉',englishName:'TUTORIAL DUNGEON',realm:RealmType.Earth,tierID:null,tierOrder:-1,totalWaves:25,startingBaseHP:20,deploymentCap:3,hpMultiplier:1,defenseMultiplier:1,speedMultiplier:1,bossName:'地脈破壞者',bossTrait:'重甲',waves:TUTORIAL_WAVES};
export function getEarthDungeon(tierID:EarthTierId):DungeonConfig {
  const tier=getEarthTier(tierID);
  return {id:`EARTH_${tierID}_DUNGEON`,name:tier.dungeonName,englishName:`${tierID} EARTH TRIAL`,realm:RealmType.Earth,tierID,tierOrder:tier.order,totalWaves:25,startingBaseHP:20,deploymentCap:tier.deploymentCap,hpMultiplier:tier.hpMultiplier,defenseMultiplier:tier.defenseMultiplier,speedMultiplier:tier.speedMultiplier,bossName:tier.bossName,bossTrait:tier.bossTrait,waves:createWaves(tier.order)};
}

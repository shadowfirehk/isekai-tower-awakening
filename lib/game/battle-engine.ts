...
export interface EnemyRuntime { runtimeId:number; enemyId:EnemyId; name:string; hp:number; maxHP:number; shieldHP:number; maxShieldHP:number; defense:number; speed:number; baseDamage:number; progress:number; boss:boolean; elite:boolean; regenerationPercent:number; supportAura:number; phase:number; role:string;visualScale:number;animationProfile:string;tierVisualVariant:string;state:'ALIVE'|'DYING'|'DEAD'; }
...
export class BattleEngine {
  ...
  private spawnEnemy(enemyId:EnemyId){const data=ENEMY_CATALOG[enemyId],waveScale=1+(this.wave-1)*.08,bossScale=data.boss?this.dungeon.waves[this.wave-1].bossMultiplier:1,elite=data.elite||data.boss&&this.dungeon.tierOrder>=2;
    ...
    this.enemies.push({runtimeId:this.id++,enemyId,name:data.boss?this.dungeon.bossName:data.name,hp:maxHP,maxHP,shieldHP,maxShieldHP:shieldHP,defense:data.defense*this.dungeon.defenseMultiplier*(elite?1.2:1),speed:data.speed*this.dungeon.speedMultiplier,baseDamage:Math.max(1,Math.round(data.baseDamage*(1+(this.wave-1)*.03))),progress:0,boss:data.boss,elite,regenerationPercent:data.regenerationPercent??(data.boss&&this.dungeon.tierOrder>=4?.008:0),supportAura:data.supportAura??0,phase:1,role:data.icon,visualScale:data.visualScale,animationProfile:data.animationProfile,tierVisualVariant:this.dungeon.tierID??'TUTORIAL',state:'ALIVE'});}
